...
function ensureGitHubBox(){
 const editor=$('chapterEditor');if(!editor||$('chapterImageUrls'))return;
 const wrap=document.createElement('div');wrap.className='full';wrap.innerHTML='<label>GitHub Pages image URLs (recommended)</label><textarea id="chapterImageUrls" rows="8" placeholder="One public GitHub Pages image URL per line"></textarea><small class="muted">Upload images to the MangaAtlas GitHub repo first, then paste their public GitHub Pages URLs here. This avoids Supabase Storage and the egress restriction.</small>';
 const grid=editor.querySelector('.formgrid');const upload=grid?.querySelector('#chapterPages')?.parentElement;grid?.insertBefore(wrap,upload||null);
}
...
async function saveGitHubUrls(){
 ensureGitHubBox();
 const urls=($('chapterImageUrls')?.value||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
 if(!urls.length)return false;
 const bad=urls.find(u=>!/^https?:\/\//i.test(u));
 if(bad){toast('Every GitHub image must be a complete http(s) URL.');return true}
 const mangaId=$('chapterManga').value,num=$('chapterNumber').value.trim(),title=$('chapterTitle').value.trim(),slug=$('chapterSlug').value.trim();
 if(!mangaId||!num||!title||!slug){toast('Manga, chapter number, chapter name and manual URL are required.');return true}
 const {data:manga,error:me}=await client.from('manga').select('cover_url').eq('id',mangaId).single();
 if(me){toast(`Could not load manga cover: ${me.message}`);return true}
 const pages=urls.map((url,i)=>JSON.stringify({url,alt:title,order:i+1}));
 const id=editingChapterId;
 const payload={manga_id:mangaId,chapter_number:Number(num),title,url_slug:slug,pages,feature_image_url:manga?.cover_url||null};
 const result=id?await client.from('chapters').update(payload).eq('id',id).select('id').single():await client.from('chapters').insert(payload).select('id').single();
 if(result.error){toast(`Chapter could not be saved: ${result.error.message}`);return true}
 toast(`${id?'Chapter updated':'Chapter added'} with ${pages.length} GitHub Pages images.`);
 if(typeof window.resetChapter==='function')window.resetChapter();else $('chapterImageUrls').value='';
 loadChapters();
 return true;
}
function controls(){
...
async function bulkFolders(files){
 ...
 const chosen=cs.filter(c=>ids.includes(c.id));const groups=new Map();
 for(const f of [...files]){const path=(f.webkitRelativePath||f.name).replace(/\\/g,'/');const m=path.match(/(?:^|\/)(?:第\s*)?(\d+(?:\.\d+)?)\s*話/);const n=m?Number(m[1]):(path.match(/chapter[-_ ](\d+(?:\.\d+)?)/i)?.[1]*1);if(n==null)continue;const candidates=chosen.filter(c=>Number(c.chapter_number)===n);if(candidates.length===1){if(!groups.has(candidates[0].id))groups.set(candidates[0].id,[]);groups.get(candidates[0].id).push(f)}else if(candidates.length>1){const hit=candidates.find(c=>path.toLowerCase().includes(String(c.manga?.title||'').toLowerCase()));if(hit){if(!groups.has(hit.id))groups.set(hit.id,[]);groups.get(hit.id).push(f)}}}
...
const oldEdit=window.editChapter;window.editChapter=async id=>{ensureGitHubBox();await oldEdit(id);try{const {data}=await client.from('chapters').select('pages').eq('id',id).single();const urls=(data?.pages||[]).map(p=>{try{return typeof p==='string'?JSON.parse(p).url:p?.url||p}catch{return p}}).filter(Boolean);if($('chapterImageUrls'))$('chapterImageUrls').value=urls.join('\n')}catch(e){}};
const oldReset=window.resetChapter;window.resetChapter=()=>{oldReset();if($('chapterImageUrls'))$('chapterImageUrls').value='';};
if(save)save.onclick=async()=>{ensureGitHubBox();if(($('chapterImageUrls')?.value||'').trim()){await saveGitHubUrls();return}toast('Paste GitHub Pages image URLs to save without Supabase Storage.');};
