/* MangaAtlas admin access-key bridge. The key is never stored in the repository. */
(function(){
  ...
  window.supabase.createClient=function(url,anon,options){
    ...
    options.global.headers={...(options.global.headers||{}),...(key()?{'x-admin-key':key()}: {})};
    const client=originalCreate(url,anon,options);
    client.auth.getSession=async()=>key()?{data:{session:{user:{id:ADMIN_USER_ID}}},error:null}:{data:{session:null},error:null};
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(!box)return;
    box.innerHTML='<h1>Manga<span class="accent">Atlas</span></h1><p class="muted">Admin access</p><label>Admin Access Key</label><input id="adminAccessKey" type="password" autocomplete="off" placeholder="Enter your admin key"><button class="btn" id="adminAccessBtn" style="width:100%;margin-top:12px">Unlock Admin</button><div id="loginStatus" class="status"></div>';
