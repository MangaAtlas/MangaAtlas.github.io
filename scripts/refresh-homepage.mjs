...
const manga = await getJson('manga?select=id,title,cover_url,country');
const chapters = await getJson('chapters?select=id,manga_id,chapter_number,url_slug,created_at,view_count&order=created_at.desc');
...
for (const [country, name] of Object.entries(countries)) {
  ...
  const content = rows.length ? rows.map(card).join('') : `<div class="empty">No ${esc(name)} chapters published yet.</div>`;
...
// Keep the homepage crawlable even if JavaScript or Supabase is temporarily unavailable.
